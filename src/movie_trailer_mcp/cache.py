"""SQLite TTL cache. Mirrors the pattern used by movie-metadata-mcp.

Keyed by canonical ``tool:{args-json}``, values are JSON blobs produced by
Pydantic ``model_dump(mode='json')``. Expired rows are dropped lazily on
read, not via a background sweeper.
"""
